...
type ServiceQuery struct {
	...
	// eager-loading edges.
	withServiceVersions *ServiceVersionQuery
...
// QueryServiceVersions chains the current query on the "serviceVersions" edge.
func (sq *ServiceQuery) QueryServiceVersions() *ServiceVersionQuery {
	query := &ServiceVersionQuery{config: sq.config}
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withServiceVersions: sq.withServiceVersions.Clone(),
...
// WithServiceVersions tells the query-builder to eager-load the nodes that are connected to
// the "serviceVersions" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ServiceQuery) WithServiceVersions(opts ...func(*ServiceVersionQuery)) *ServiceQuery {
	query := &ServiceVersionQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withServiceVersions = query
	return sq
}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context) ([]*Service, error) {
	...
	if query := sq.withServiceVersions; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Service)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ServiceVersions = []*ServiceVersion{}
		}
		query.withFKs = true
		query.Where(predicate.ServiceVersion(func(s *sql.Selector) {
			s.Where(sql.InValues(service.ServiceVersionsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.service_service_versions
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "service_service_versions" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "service_service_versions" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ServiceVersions = append(node.Edges.ServiceVersions, n)
		}
	}
